...
import { getCliVersion, isDir, isMD } from "../shared/fsUtils.mjs";
...
const replaceDot = (s) => s.replace(/`/g, "");
const replaceVersion = (s) => s.replace(/\*\*\*.+\*\*\*/g, "").trim();
const replaceUnderline = (s) => s.replace(/_/g, "");
...
function compileWebTypes(table, webTypes, componentName, varletConfig) {
  ...
  webTypes.contributions.html.tags.push({
    name: `${varletConfig == null ? void 0 : varletConfig.namespace}-${componentName}`,
    attributes,
    events,
    slots
  });
}
function compileMD(path, webTypes, varletConfig, options) {
...
function compileDir(path, webTypes, varletConfig, options) {
...
function compileLanguageMD(varletConfig, options) {
  const webTypes = {
    $schema: "https://raw.githubusercontent.com/JetBrains/web-types/master/schema/web-types.json",
    framework: "vue",
    version: getCliVersion(),
    name: varletConfig == null ? void 0 : varletConfig.title,
    contributions: {
      html: {
        tags: [],
